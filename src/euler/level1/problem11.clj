...
(defn diagonals-of [grid]
  (let [size (count grid)
        mix (apply interleave grid)
        full-diags (map (fn [i] (take-nth (inc size) (drop i mix))) (range (inc size)))
        half1 (map #(take (- size %) (nth full-diags %)) (range (inc size)))
        half2 (map #(drop (- size %) (nth full-diags %)) (range (inc size)))
        whole (concat (reverse half1) (reverse half2))]
    (filter seq whole)))
